...
//view scheduled function
$outofstockorders = "select * from orders where outofstock=1";
$outofstock_orders= mysql_query($outofstockorders,$dblink);
//all orders
...
<div align="left">
<label for="Admin"><center><h2><b>Out Of Stock Orders</b></h2><center></label></div>
...
while ($line = mysql_fetch_assoc($outofstock_orders)){
echo '<tr onClick="HighLightTR(this);"><td>'.$line['order_id'].' </td><td>'.$line['fname'].'  </td><td>'.$line['lname'].' </td><td>'. $line['item'].' </td><td>'.$line['Rental_term'].'
	</td><td>'.$line['pnumber'].' </td><td>'.$line['email'].'</td></tr>';
}
?>
</table>
</div>
