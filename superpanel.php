...
//countrow
$resultcount = mysql_query("SELECT * FROM orders where  returned=1 or checkedout=1 ", $dblink);
...
<!-- //////////////////////////////////////////////////////                     /////////////////////////////////////////////////////////////// -->
<table>
<th>  Number of Total Paid Orders </th>
<tr>
...
echo "<table>";  
echo "<tr>";  
echo "<th>Sum of total Paid Orders</th>";  
$result = mysql_query("SELECT SUM(amtPaid)   
FROM orders  
WHERE returned=1 or checkedout=1 and comp=0");  
...
<div class="orders">
<table class="hoverTable" id="display" align="left" width=100% border="3" cellpadding ="7" cellspacing ="7" text-align:"left">
<form id="orders_form" action="<?= $_SERVER['PHP_SELF'] ?>" method='post'>
<div align="left">
<label for="Admin"><center><h2><b>All Info On All Orders</b></h2><center></label></div>
<th>Order</th>
<th>First Name</th>
<th>Last Name</th>
<th>Otter ID</th>
...
while ($line = mysql_fetch_assoc($resultall_orders)){
echo '<tr onClick="HighLightTR(this);"><td>'.$line['order_id'].' </td><td>'.$line['fname'].'  </td><td>'.$line['lname'].'</td><td>'.$line['otterid'].' </td><td>'. $line['item'].' </td><td>'.$line['Rental_term'].'
	</td><td>'.$line['pnumber'].' </td><td>'.$line['email'].'</td><td>'.$line['serialnumber'].'</td><td>'.$line['amtPaid'].'</td><td>'.$line['latefee'].'</td><td>'.$line['dateout'].'</td><td>'.$line['returndate'].'</td><td>'.$line['returned'].'</td><td>'.$line['checkedout'].'</td><td>'.$line['comp'].'</td><td>'.$line['staff'].'</td></tr>';
}
?>
...
<div align="left">
<?= ((!empty($order)) ? '<div class="error">' : '') ?><label for = "order">Order: </label><input type="text" name="order" size="8" maxlength="40"><br>
<?= ((!empty($date)) ? '<div class="error">' : '') ?><label for = "date">Appt: </label><input type="text" name="date" size="15" maxlength="60">
</div>
<div align="center">
