...
$macorders = "select * from orders where item like'Macbook' and returned=1 or item like'Macbook' and checkedout=1 ";
$macall_orders= mysql_query($macorders,$dblink);
$clickerorders = "select * from orders where item like'clicker' and returned=1 or item like'clicker' and checkedout=1 ";
$clickerall_orders= mysql_query($clickerorders,$dblink);

$resultcount = mysql_query("SELECT * FROM orders where item like'clicker' and returned=1 or item like'clicker' and checkedout=1 ", $dblink);
$num_rows = mysql_num_rows($resultcount);

//SELECT SUM(column_name) FROM table_name;
$sumcount = "SELECT SUM(amtPaid) FROM orders where item like'clicker' and returned=1 or item like'clicker' and checkedout=1 ";
$sum_orders= mysql_result($sumcount,$dblink);
...
<!-- //////////////////////////////////////////////////////                     /////////////////////////////////////////////////////////////// -->
<table>
<th>  Number of Total Clicker Orders  </th>
<tr>
<td align='center' width='200' >
<?php
echo "  &nbsp&nbsp $num_rows &nbsp&nbsp ";
?>
</td>
</tr>
</table>
...
echo "<table>";  
echo "<tr>";  
echo "<th>Sum of total Clicker orders</th>";  
$result = mysql_query("SELECT SUM(amtPaid)   
FROM orders  
WHERE item like'clicker' and returned=1 or item like'clicker' and checkedout=1");  
while($row=mysql_fetch_array($result))  
{  
echo "<tr>";  
echo "<td align='center' width='200'>" . $row['SUM(amtPaid)'] . "</td>";  
echo "</tr>";  
}  
echo "</table>";  
...
<label for="Admin"><center><h2><b>Clicker Orders</b></h2><center></label>
...
while ($line = mysql_fetch_assoc($clickerall_orders)){
echo '<tr onClick="HighLightTR(this);"><td>'.$line['order_id'].'</td><td>'.$line['fname'].'</td><td> '.$line['lname'].'</td><td>'. $line['item'].' </td><td>'.$line['Rental_term'].'</td><td>'.$line['pnumber'].'</td><td>'.$line['email'].'</td><td>'.$line['amtPaid'].'</td><td>'.$line['returned'].'</td><td>'.$line['serialnumber'].'</td><td>'.$line['returndate'].'</td><td>'.$line['dateout'].'</td</tr>';
