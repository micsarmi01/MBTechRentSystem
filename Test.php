...
if(isset($_POST['latefee'])){
	...
	if(empty($Error)){
		...
		$result = "UPDATE orders SET latefee= '$_POST[latefee]' WHERE order_id = '$_POST[order]'";
...
echo '<tr onClick="HighLightTR(this);"><td>'.$line['order_id'].'</td><td>'.$line['fname'].'</td><td> '.$line['lname'].'</td><td>'. $line['item'].' </td><td>'.$line['Rental_term'].'</td><td>'.$line['pnumber'].'</td><td>'.$line['email'].'</td><td>'.$line['desired'].'</td></tr>';
...
<?= ((!empty($order)) ? '<div class="error">' : '') ?><label for = "order">Order:</label><input type="text" name="order" size="8" maxlength="40"><br>
<br>
<label for = "latefee">Amount Late Fee Paid:</label><input type="text" name="latefee" size="8" maxlength="40"><br>
<br>
</div>
<div align="center">
